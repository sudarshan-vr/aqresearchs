...
function BioImg({ member }) {
  const [src, setSrc] = useState(member.img);
  const [showInitials, setShowInitials] = useState(false);

  const handleError = () => {
    if (src !== member.fallbackImg && member.fallbackImg) {
      setSrc(member.fallbackImg);
    } else {
  ...
  return (
    <>
      {!showInitials && (
        <img src={src} alt={member.name} className="bio-card__img" onError={handleError} />
      )}
      {showInitials && (
        <div className="bio-card__initials" style={{ display: 'flex' }}>{initials}</div>
      )}
    </>
  );
}
        ...
        <div className="container about-overview__inner">
          <div className="about-overview__img-col">
            <div className="about-overview__img-wrap">
              <img src="/assets/picture.jpg" alt="AQ Research"
                className="about-overview__img"
                onError={(e) => {
                  if (e.target.src !== '/picture.jpg') {
